...
public class FirstController {
    ...
    * */
    @GetMapping("regist")       // GET 방식은 연결할때, 조회할때 사용
    public void regist() {}
    ...
    * */
    ...
    public String registMenu(Model model, WebRequest request) {
        ...
        String message = name + "을(를) 신규 메뉴 목록의 " + categoryCode + "번 카테고리에 " + price + "원으로 등록 하셧습니다!";
    ...
    *
    *       전달하는 form의 name 속성이 일치하는 것이 없는 경유 400에러가 발생하는데 이는 required 속성의 기본 값이 true이기 때문이다.
    ...
    * 화면에 출력해보면 모델에 담겨진 값을 확인할 수 있다.
    *
    * 경우에 따라 폼에서 입력한 값을 타음 화면으로 바로 전달해야 하는 경우가 발생하는데 이 때 유용하게 사용할 수 있다.
    ...
    *   Model영역에 해당 key로 값이 추가되는 경우 session에 자동 등록 한다.
    * */
