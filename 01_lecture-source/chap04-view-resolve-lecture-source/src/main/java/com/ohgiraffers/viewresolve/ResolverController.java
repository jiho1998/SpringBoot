package com.ohgiraffers.viewresolve;
...
public class ResolverController {
    ...
    * 하지만 스프링에서는 RedirectAttribute 타입을 통해 redirect시 속성 값을 저장할 수 있도록 하는 기능을 제공한다.
    * */
    ...
    public String stringRedirectAttribute(RedirectAttributes rttr) {
        ...
        * 세션에 임시로 값을 담고 소명하는 방식이기 때문에 session에 동일한 키값이 존재하지 않아야 한다. */
    ...
    public ModelAndView modelAndViewReturning(ModelAndView mv) {
        ...
        /* 모델과 뷰를 합친 개념이다.
        *  핸들러 어댑터가 핸들러 메소드를 호출하고 반환받는 문자열을 ModelAndView로 만들어 dispatcherServlet에 반환한다.
